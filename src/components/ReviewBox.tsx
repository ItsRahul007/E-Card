  ...
  return (
    <div className="w-11/12 ml-4 border-b pb-2">
      ...
      <div className="h-10 w-full flex items-center justify-start">
        ...
        <span className="text-sm ml-3 font-semibold mb-1 capitalize">
          {pikRatingText()}
        </span>
      </div>
      ...
      <div>
        <span>
          {!isFullText && reviewText.length > 300 ? (
            <pre className="font-inter">{reviewText.slice(0, 300) + "..."}</pre>
          ) : (
            <pre className="font-inter">{reviewText}</pre>
          )}
          <span
          ...
          >
            {!isFullText && reviewText.length > 300
              ? "read more"
              : [reviewText.length > 300 && "show less"]}
          </span>
        </span>
      </div>
    </div>
