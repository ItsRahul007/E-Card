...
const SingleNewReleaseItem: React.FC<I_SingleNewReleaseItem> = ({
  imgUrl,
  price,
  productId,
  product_name,
  ...
  return (
    <Link
      ...
      target="_blank"
      className="h-full w-full flex flex-col gap-2 bg-lightBg items-center justify-center py-3"
    >
      ...
      <div className="flex flex-col gap-1 text-center px-1 max-w-full md:w-auto truncate overflow-hidden text-ellipsis">
        ...
        <span className="text-lightColor text-sm font-medium">${price}</span>
      </div>
    </Link>
  );
};
