...
const ItemCard: React.FC<I_ItemCard> = ({
  ...
  return (
    <div className="border border-rootBg sm:h-72 min-[390px]:h-60 h-52 sm:w-52 min-[390px]:w-44 w-36 flex flex-col items-center gap-1 overflow-hidden rounded-md shadow bg-rootBg cursor-pointer hover:translate-y-[-2px] col-span-1 duration-200">
      ...
      <div className="h-1/4 w-full flex flex-col items-center justify-start min-[390px]:gap-[2px]">
        ...
        <div className="w-[90%] text-base text-rootColor ml-1 font-semibold flex items-center justify-between">
          <span>${Math.round(current_price)}</span>
          <span>
            {ratings.length > 0 ? (
              generateStars()
            ) : (
              <span className="text-xs text-lightColor">No Ratings</span>
            )}
          </span>
        </div>
      </div>
