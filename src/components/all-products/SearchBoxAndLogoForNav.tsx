  ...
  return (
    <span className="h-full xl:w-[30rem] min-[1281px]:flex hidden justify-end items-center gap-8">
      ...
      <InputCompo
        ...
        className="p-3 rounded-md text-sm outline-none text-rootColor w-64 placeholder:font-sans font-sans border-0 !bg-lightBg"
        onChange={(e) => setInputValue(e.target.value)}
        value={inputValue}
        onEnter={() => {
          if (inputValue.length) {
            router.push(
              "/products/search-products?search=" + inputValue.toLowerCase()
            );
          }
        }}
      />
