...
const AddressCompo: FC<T_AddressCompo> = (props) => {
  ...
  const handleUpdate = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      setAddress.mutate(
        { addresses: inputValues, method: "put" },
        {
          onSuccess: () => {
            refetch();
  ...
  const handleDelete = () => {
    setAddress.mutate(
      { addresses: inputValues, method: "delete" },
      {
        onSuccess: () => {
          refetch();
  ...
  return (
    <>
      {!isEditFormActive ? (
        <div className="h-auto w-full flex flex-col gap-2 px-6 py-5 text-rootColor relative">
          {/* three dots*/}
          {!props.noEditOption && (
            <div className="absolute top-2 right-2 text-lightColor peer z-[1]">
              <i className="ri-more-2-fill text-xl"></i>
            </div>
          )}

          {/* options */}
          <div className="absolute top-2 right-2 p-3 peer-hover:!flex hover:!flex flex-col gap-2 rounded text-sm border shadow-md text-rootColor z-[2] hidden bg-rootBg">
            <span
          ...
          <div className="text-xs sm:text-sm capitalize">{props.address}</div>
        </div>
      ) : (
        <EditAddressForm
