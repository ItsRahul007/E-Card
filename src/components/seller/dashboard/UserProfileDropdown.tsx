...
const UserProfile: React.FC<I_UserProfile> = ({ userAvatar }) => {
  return (
    <Menu as="div" className="relative ml-3">
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white dark:bg-zinc-900 text-rootColor py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          {userNavigation.map((item) => (
            <Menu.Item key={item.href + "user-profile-dropdown"}>
              <Link
                href={item.href}
                className={
                  "hover:bg-zinc-100 dark:hover:bg-zinc-800 block px-4 py-2 text-sm"
                }
              >
                {item.name}
              </Link>
