...
export async function POST(req: NextRequest) {
    try {
        ...
        if (isBrandNameAlreadyExists || isContactEmailAlreadyExists || isContactNumberAlreadyExists) {
            const response = NextResponse.json({
                success: false,
                error: isBrandNameAlreadyExists ? "Brand name already exists" : isContactEmailAlreadyExists ? "Contact email already exists" : "Contact number already exists",
            }, { status: 400 });
