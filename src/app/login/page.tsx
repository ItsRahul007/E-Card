    ...
    <div className={style.login_image}>
      ...
      <form className={style.glass}>
        ...
        <div className="w-full h-[60%] flex flex-col items-center gap-3 mt-4">
          ...
          {signup && <input type="password" placeholder="Confirm password" className="bg-transparent border-solid border-b-2 text-[#a2acba] outline-0 p-2 px-4" required />}
          <Button
          ...
          />
          <p className="text-white">{signup? "Already" : "Don't"} have an account? <Link href={`/${signup? "login" : "signup" }`} className="text-blue-800 hover:decoration-blue-800 hover:underline font-semibold">{signup? "Login" : "Signup" }</Link></p>
