  ...
  async function onSubmit(e: any) {
    ...
    if (!signup) {
      try {
        ...
        toast.error(errorResponse ? errorResponse.data.error : ErrorMessage);
        console.log(error);
      }
    } else {
  ...
  return (
    <form onSubmit={onSubmit}>
      <div className="h-[20%] w-full flex flex-col gap-2 items-center justify-center text-white">
        <h4 className={"text-4xl font-poppins font-bold"}>
          {signup ? signup : "Login"}
        </h4>
      ...
      <div className="w-full h-[60%] flex flex-col items-center gap-3 mt-4">
        ...
        <p className="text-white">
          ...
          <Link
            ...
            className="text-blue-600 hover:decoration-blue-800 hover:underline font-semibold"
          >
            {signup ? "Login" : "Signup"}
          </Link>
        </p>
