...
export async function generateMetadata({ searchParams }: I_AllProduct): Promise<Metadata> {
  const ProductDescription = searchParams.search ? await getProductDescription(searchParams.search) : undefined;

  const description = ProductDescription ? ProductDescription : defaultDescription;
